...
class ContractCategorizer:
    ...
    def detect_category(file_path: Path) -> Optional[str]:
        ...
        if parent_dir in ['authentication', 'authorization', 'dataplane', 'factory', 'registration', 'telemetry', 'subscription']:
            return parent_dir

        # Check grandparent directory for versioned contracts (e.g., dataplane/v1.0.0/file.yaml)
        if grandparent_dir and grandparent_dir in ['authentication', 'authorization', 'dataplane', 'factory', 'registration', 'telemetry', 'subscription']:
