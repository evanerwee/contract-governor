...
class Manifest:
    ...
    def add_contract(self, path: str, category: str, version: str,
                     stipulation_path: str, content: bytes) -> None:
        """Add contract entry to manifest."""
        sha256 = hashlib.sha256(content).hexdigest()

        self.contracts.append(ContractManifestEntry(
            path=path,
            sha256=sha256,
            category=category,
            version=version,
            stipulation=stipulation_path
        ))

        self.links.append(ContractStipulationLink(
            contract=path,
            stipulation=stipulation_path
        ))

    def add_stipulation(self, path: str, category: str, version: str,
                       content: bytes) -> None:
        """Add stipulation entry to manifest."""
        sha256 = hashlib.sha256(content).hexdigest()

        self.stipulations.append(StipulationManifestEntry(
            path=path,
            sha256=sha256,
            category=category,
            version=version
