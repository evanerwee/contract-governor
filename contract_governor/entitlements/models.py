...
class SpiceDBRelationship(BaseModel):
    ...
    resource: str = Field(
        ..., description="Resource identifier (e.g., 'api_operation:lexical-graph_prompts_status_get')"
    )
    ...
    def to_spicedb_format(self) -> Dict[str, str]:
        ...
        return {"resource": self.resource, "relation": self.relation, "subject": self.subject}
...
class OperationEntitlement(BaseModel):
    ...
    query_parameters: List[str] = Field(
        default_factory=list, description="List of query parameter names from OpenAPI spec"
    )
    query_parameter_examples: Dict[str, str] = Field(
        default_factory=dict, description="Example values for query parameters from OpenAPI spec"
    )
    query_parameter_required: Dict[str, bool] = Field(
        default_factory=dict, description="Whether each query parameter is required"
    )
    query_parameter_types: Dict[str, str] = Field(
        default_factory=dict, description="Type of each query parameter (string, integer, etc.)"
    )
    body_parameters: List[str] = Field(
        default_factory=list, description="List of body parameter names from request body schema"
    )
    body_parameter_examples: Dict[str, Any] = Field(
        default_factory=dict, description="Example values for body parameters"
    )
    body_parameter_required: Dict[str, bool] = Field(
        default_factory=dict, description="Whether each body parameter is required"
    )
    body_parameter_types: Dict[str, str] = Field(
        default_factory=dict, description="Type of each body parameter (string, array, boolean, etc.)"
    )
    ...
    request_body_schema: Optional[Dict[str, Any]] = Field(
        None, description="Request body schema for POST/PUT operations"
    )
...
class ContractManifest(BaseModel):
    ...
    operations: List[OperationEntitlement] = Field(default_factory=list, description="List of operations")
