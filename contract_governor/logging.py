...
"""Document Graph Logging Configuration.

This module provides customized logging configuration for the document graph toolkit,
including formatters, filters, and utility functions for setting up logging.
It supports filtering by module name and message content, as well as customizing
log output formats.
"""
...
class ModuleFilter(logging.Filter):
    ...
    def __init__(
        ...
        This filter allows for fine-grained control over which log messages are displayed
        based on module names and message content.

        Args:
            included_modules (Dict[LoggingLevel, Union[str, List[str]]], optional): Modules to include at each level. Defaults to None.
            excluded_modules (Dict[LoggingLevel, Union[str, List[str]]], optional): Modules to exclude at each level. Defaults to None.
            included_messages (Dict[LoggingLevel, Union[str, List[str]]], optional): Message prefixes to include at each level. Defaults to None.
            excluded_messages (Dict[LoggingLevel, Union[str, List[str]]], optional): Message prefixes to exclude at each level. Defaults to None.
        """
        super().__init__()
...
# Configure default logging on module import to prevent DEBUG spam
